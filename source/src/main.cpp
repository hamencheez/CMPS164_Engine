...
bool mouseDownL, mouseDownR, bBox, smoothShade, freeLook;
...
void checkForBallInHole() {
	//Radius of hole should be about 8
	float x = abs(golfBall->position->getX() - resManager->getCup().x);
	float z = abs(golfBall->position->getZ() - resManager->getCup().z);
...
void updateBall() {
	...
	if (golfBall->isMoving) {
		...
		currTile = resManager->getTile(golfBall->position, physicsManager);
...
//	cameraManager->update(Camera::TRACK_BALL);
...
//				resManager->getBallNorms().data(), GL_DYNAMIC_DRAW);
    ...
    //draw the vertices/normals we just specified.
    glDrawArrays(GL_TRIANGLES, 0, resManager->getNumArrowVerts());
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
    	...
    	case 114: modelViewBall = defaultView; golfBall->resetBall(); modelViewBall = glm::translate(defaultView, glm::vec3(
										resManager->getTee().x, resManager->getTee().z, resManager->getTee().y)); break;
...
void setupGL() {
	...
	cameraManager->setCamTargPos(0, 0, 0);
	...
	ballPos = glm::vec3(resManager->getTee().x, resManager->getTee().y, resManager->getTee().z);
	golfBall->velocity = new Vector3(0, 0, 0);
	currTile = resManager->getTile(resManager->getTee().id);
...
int main(int argc, char **argv) {
	...
	resManager->readTileFile(argv[1]);
